...
$(document).ready(function(){
	...
	/*++++++++++++++++++++++++++++++++++++
		Conferences page categorization
	++++++++++++++++++++++++++++++++++++++*/
	
	
	$('div#conf-grid').mixitup({
		layoutMode: 'list',
		easing : 'snap',
		transitionSpeed :600,
		onMixEnd: function(){
	...
	}).on('click','div.confmain',function(){
		var $this = $(this), 
			$item = $this.closest(".item");
		
		$item.find('div.confdetails').slideToggle(function(){
			$this.children("i").toggleClass('icon-collapse-alt icon-expand-alt');
		},function(){
			$this.children("i").toggleClass('icon-expand-alt icon-collapse-alt');
		});
	...
	$("[name=cd-dropdown]").on("change",function(){
		var item = this.value;		
		$('div#conf-grid').mixitup('filter',item);
	});
...
$(".ball").click(function () {
    ...
    $water.slideToggle(600, function () {
